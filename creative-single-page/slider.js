function Slider() {
	...
	function matchMedia() {
		...
		if (mediaQueryMAX.matches) {
			/* document.querySelector(".slider-content").style.maxWidth =
				currentContent.clientWidth + "px"; */
	...
	function addClones() {
		...
		slideWrapper.style.top = -currentSlide.offsetHeight + "px";
		contentWrapper.appendChild(cloneFirstContent);
		contentWrapper.insertBefore(cloneLastContent, currentContent);
		contentWrapper.style.left = -currentContent.clientWidth + "px";
	}

	function onResize() {
		window.addEventListener("resize", () => {
			slideWrapper.style.top = -currentSlide.offsetHeight * index + "px";
			contentWrapper.style.left =
				-currentContent.clientWidth * index + "px";
	...
	function moveSlide(direction) {
		...
		slideWrapper.classList.add("transitionTop");
		contentWrapper.classList.add("transitionLeft");
			...
			if (index === 0) {
				...
				slideWrapper.addEventListener("transitionend", () => {
					slideWrapper.style.top =
						-currentSlide.offsetHeight * index + "px";
					contentWrapper.style.left =
						-currentContent.clientWidth * index + "px";
				});
			}
		...
		slideWrapper.addEventListener("transitionend", () => {
			slideWrapper.classList.remove("transitionTop");
			contentWrapper.classList.remove("transitionLeft");
		});
	}
